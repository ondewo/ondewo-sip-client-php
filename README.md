...
The client is published on [Packagist](https://packagist.org/packages/ondewo/sip-client-php) as
**`ondewo/sip-client-php`** and is installed like any other composer package:

```bash
composer require ondewo/sip-client-php
```

The client version tracks the ONDEWO SIP API in major and minor version, so pin the minor series that
...
* **`ext-grpc` is a hard requirement**, not an optional extra — every generated `<Service>Client`
  ...
  (see [Requirements](#requirements) above); `composer require ondewo/sip-client-php --ignore-platform-req=ext-grpc`
...
* **There is no build step.** Packagist serves the tree of a git tag verbatim and the generated stubs are
  ...
  `composer require`, `vendor/ondewo/sip-client-php/src/` already holds every stub and the autoloader is
  generated by composer itself. Nothing needs docker, protoc or the `ondewo-sip-api` submodule.
...
use Ondewo\Sip\Auth\BearerTokenAuthenticator;   // hand-written, from auth/
use Ondewo\Sip\SipClient;                       // generated, from src/
```
...
1. **Submit the package once.** Log in to [packagist.org](https://packagist.org/) with the ONDEWO account and
   use *Submit* with the repository URL `https://github.com/ondewo/ondewo-sip-client-php`. Packagist reads
...
| `check_version_agreement` | `ONDEWO_SIP_VERSION`, `composer.json`'s `name` and `version`, the `RELEASE.md` entry and — on a tag — the tag name drifting apart. A tag that disagrees with the `version` field makes Packagist publish the tree under a version nobody tagged |
